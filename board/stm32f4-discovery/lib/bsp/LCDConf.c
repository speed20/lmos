...
#define LCD_LAYER1_FRAME_BUFFER  ((uint32_t)0xD0050000)
...
static volatile int32_t         layer_pending_buffer[GUI_NUM_LAYERS];
...
static void InvertAlpha_SwapRB(LCD_COLOR * pColorSrc, LCD_COLOR * pColorDst, uint32_t NumItems) 
{
  ...
  {
    Color = *pColorSrc++;
    *pColorDst++ = ((Color & BLUE_MASK) << RED_SHIFT)         /* Swap red <-> blue */
                 |  (Color & GREEN_MASK)                /* Green */
                 | ((Color & RED_MASK) >> RED_SHIFT)         /* Swap red <-> blue */
...
static void DMA_AlphaBlendingBulk(LCD_COLOR * pColorFG, LCD_COLOR * pColorBG, LCD_COLOR * pColorDst, uint32_t NumItems) 
{
  ...
  /* Set up pixel format */
  DMA2D->FGPFCCR = LTDC_Pixelformat_RGB565;       /* Foreground PFC Control Register (Defines the FG pixel format) */
  DMA2D->BGPFCCR = LTDC_Pixelformat_RGB565;       /* Background PFC Control Register (Defines the BG pixel format) */
...
static LCD_COLOR DMA_MixColors(LCD_COLOR Color, LCD_COLOR BkColor, U8 Intens) 
{
  ...
  DMA2D->FGPFCCR = LTDC_Pixelformat_RGB565
                 | (1UL << 16)
                 | ((uint32_t)Intens << 24);
  DMA2D->BGPFCCR = LTDC_Pixelformat_RGB565
                 | (0UL << 16)
...
static void DMA_Color2IndexBulk(LCD_COLOR * pColor, void * pIndex, uint32_t NumItems, U8 SizeOfIndex, uint32_t PixelFormat) 
{
  ...
  /* Use DMA2D for the conversion */
  DMA_ConvertColor(aBufferDMA2D, pIndex, LTDC_Pixelformat_RGB565, PixelFormat, NumItems);
}
