...
#define sampleFreq	380.0f		// sample frequency in Hz
...
void getEulerAngles(float *euler) {
  ...
  if (test > 0.499) {
    ...
    euler[0] = atan2(2*q2*q0 - 2*q1*q3, 1 - 2*q2*q2 - 2*q3*q3)*180/PI;
    euler[1] = asin(2*q1*q2 + 2*q3*q0)*180/PI;
    euler[2] = atan2(2*q1*q0 - 2*q2*q3, 1 - 2*q1*q1 - 2*q3*q3)*180/PI;
  }
}
