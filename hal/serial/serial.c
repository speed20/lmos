...
void serial_init(uint8_t port_num, uint32_t baudrate)
{
	...
	switch(port_num) {
		case 0:
			...
			DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&(serial_port[port_num]->DR));
	...
	USART_OverSampling8Cmd(serial_port[port_num], ENABLE);
	...
	USART_Cmd(serial_port[port_num], ENABLE);
}
...
void serial_write(uint8_t port_num, uint8_t *buf, uint32_t len)
{
	...
	for (i=0; i<len; i++) {
		USART_SendData(serial_port[port_num], buf[i]);
		while (USART_GetFlagStatus(serial_port[port_num], USART_FLAG_TC) != SET) ;
	}
}
